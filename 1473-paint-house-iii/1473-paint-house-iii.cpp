...
int helper(vector<int>& houses, vector<vector<int>>& cost, int target, int idx, int count, int prev){
        //base cases
        ...
        if(houses[idx]){
          //if prev house is of different color, increase the count by 1
            if(houses[idx]!=prev) 
              minCost = helper(houses,cost,target,idx+1,count+1,houses[idx]);
            else 
              minCost = helper(houses,cost,target,idx+1,count,houses[idx]);
            ...
            for(int j=0;j<cost[0].size();j++){
                ...
                //if prev house is of different color, increase the count by 1
                if((j+1)!=prev) tmp = cost[idx][j] + helper(houses,cost,target,idx+1,count+1,j+1);
                
                else tmp = cost[idx][j] + helper(houses,cost,target,idx+1,count,j+1);
